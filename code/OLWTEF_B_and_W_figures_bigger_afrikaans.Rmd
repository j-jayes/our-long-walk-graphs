...
df_pov_comp_lables <- df_pov_comp_lables %>% 
  mutate(label = case_when(
    label == "Living on less than $1.9 per day" ~ "Leef met minder as $1.9 per dag",
    label == "Living on less than $1 per day" ~ "Leef met minder as $1 per dag",
    TRUE ~ "Leef met minder as $2 per dag"
  )) 
...
df_pov_comp %>% 
  ...
  labs(x = NULL, 
       y = "Die deel van die wêreldbevolking wat in absolute armoede leef",
       # title = "Share of population living in absolute poverty, 1820-2015",
       # subtitle = "GDP per capita adjusted for price changes over time (inflation) and price differences between countries - it is measured in international-$ in 2011 prices",
       colour = NULL,
       caption = "Bron: Our World in Data, Bourguinon en Morrison (2002), Wêreldbank (2015)") +
  guides(colour = guide_legend(nrow=3,byrow=TRUE))
...
```{r}
df_pov_graph <- df_pov_graph %>%
  mutate(country_name_afr = case_when(
    country_name == "Europe & Central Asia" ~ "Europa en Sentraal-Asië",
    country_name == "Middle East & North Africa" ~ "Midde-Ooste en Noord-Afrika",
    country_name == "South Asia" ~ "Suid-Asië",
    country_name == "Sub-Saharan Africa" ~ "Sub-Sahara-Afrika ",
    country_name == "World" ~ "Wêreld",
    country_name == "South Africa" ~ "Suid-Afrika",
  ))


jpeg(
    filename="graphs_afr/33.1_v1.jpeg",
    width=8,
    height=6,
    units="in",
    res=1000)
...
df_pov_graph %>% 
  ...
  geom_text_repel(data = df_pov_graph %>% 
                    group_by(country_name) %>% 
                    filter(year == max(year)), 
                  aes(year, stat, label = country_name_afr),
                  size = 5) +
  scale_y_continuous(labels = scales::percent_format(scale = 1)) + 
  scale_colour_grey(start = 0.8, end = 0.2) +
  ...
  theme(legend.position = "none") +
  labs(y = "Persentasie van bevolking wat in ekstreme armoede leef", 
       x = NULL,
       # title = "Share of population living in extreme poverty by region",
       # subtitle = "GDP per capita adjusted for price changes over time (inflation) and price differences between countries \n It is measured in international-$ in 2011 prices",
       caption = "Bron: Wêreldbank se Wêreldontwikkelingsaanduiders")
...
```{r}
jpeg(
    filename="graphs_afr/33.1_v2.jpeg",
    width=8,
    height=6,
    units="in",
    res=1000)
...
df_pov_graph %>% 
  ggplot(aes(year, stat, colour = country_name_afr, lty = country_name_afr, shape = country_name_afr)) +
  geom_line(size = 1.4) +
  geom_point(size = 3) +
  scale_y_continuous(labels = scales::percent_format(scale = 1)) + 
  scale_colour_grey(start = 0.8, end = 0.2) +
  ...
  # scale_colour_brewer(type = "seq", palette = "Greys",  name = "Region") +
  theme(legend.position = "bottom") +
  labs(y = "Persentasie van bevolking wat\nin ekstreme armoede leef", 
       x = NULL,
       shape = NULL,
       colour = NULL,
       lty = NULL,
...
jpeg(
    filename="graphs_afr/2.1.jpeg",
    width=8,
    height=6,
    units="in",
    res=1000)

# South Africa, Kenya, Benin, Israel, Italy, China, Mexico, Colombia, Brazil
# Suid-Afrika, Kenia, Benin, Israel, Italië, China, Mexiko, Colombië, Brasilië

ashraf_and_galor %>% 
  filter(country_code %in% c("ZAF", "KEN", "BEN", "ISR", "ITA", "CHN", "MEX", "COL", "BRA")) %>% 
  mutate(label_afr = case_when(
   country_code == "ZAF" ~ "Suid-Afrika",
      country_code == "KEN" ~ "Kenia",
   country_code == "BEN" ~ "Benin",
   country_code == "ISR" ~ "Israel",
   country_code == "ITA" ~ "Italië",
   country_code == "CHN" ~ "China",
   country_code == "MEX" ~ "Mexiko",
   country_code == "COL" ~ "Colombië",
   country_code == "BRA" ~ "Brasilië"
  ...
  geom_text_repel(aes(x, y, label = label_afr),
                  size = 4) +
  scale_shape_manual(values = seq(15,20, by = 1)) +
  labs(x = "Genetiese diversiteit",
       y = "Logaritme van bevolkingsdigtheid in 1500",
       caption = "Bron: Ashraf en Galor (2013)") +
  # theme_classic() +
  theme(legend.title = element_blank())  +
  theme(legend.position = "bottom")
...
bm_labels <- tibble(
  x = c(22, 36, 12),
  y = c(-3, -25, 9),
  label = c("Verspreiding van Bantoesprekers\nvanaf 3000 v.C.", "Verspreiding van Bantoesprekers \nteen die jaar 1500 v.C.", "Bantoe-tuisland \nteen 4000 v.C."))
```

```{r}

jpeg(
    filename="graphs_afr/4.1.jpeg",
    width=8,
    height=6,
    units="in",
    res=1000)
...
ggplot(data = world) +
  ...
  labs(x = "",
       y = "",
       caption = "Bron: Wikipedia – ‘Bantu expansion’") +
  theme_void()
...
# Christen
# Hebreeus of Joods
# Nie-Abrahamitiese gelowe
# Israel en Jordanië
# Moslem

timeline_of_jerusalem <- timeline_of_jerusalem %>%
  mutate(religion_afr = case_when(
    religion == "Christian" ~ "Christen",
    religion == "Hebrew or Jewish" ~ "Hebreeus of Joods",
    religion == "Division - Israel and Jordan" ~ "Israel en Jordanië",
    religion == "Muslim" ~ "Moslem",
    religion == "Non-Abrahamic religion" ~ "Nie-Abrahamitiese gelowe"
  ))
...
timeline_of_jerusalem <- timeline_of_jerusalem %>%
  mutate(period_afr = case_when(
    period == "Canaanite" ~ "Kanaän",
    period == "Jebusite" ~ "Jebusieties",
    period == "Judah" ~ "Juda",
    period == "Neo-Babylonian (1)" ~ "Neo-Babilonië (1)",
    period == "Neo-Babylonian (2)" ~ "Neo-Babilonië (2)",
    period == "Diadochi (Ptolemaic/ Seleucid)" ~ "Diadochi",
    period == "Roman" ~ "Romeins",
    period == "Persian (Sassanid)" ~ "Persies",
    period == "Rashidun" ~ "Rashidunies",
    period == "Abbasid (1)" ~ "Abbasidies (1)",
    period == "Abbasid (2)" ~ "Abbasidies (2)",
    period == "Fatimid (1)" ~ "Fatimidies (1)",
    period == "Fatimid (2)" ~ "Fatimidies (2)",
    period == "Ayyubid (1)" ~ "Ayyubidies (1)",
    period == "Ayyubid (2)" ~ "Ayyubidies (2)",
    period == "Ottoman" ~ "Ottomaans",
    period == "Israel and Jordan" ~ "Israel en Jordanië"
  ))
...
timeline_of_jerusalem %>%
  ...
  geom_segment(aes(y = period, yend = period, x = date_start, xend = date_end, colour = religion_afr), cex = 3) +
  # geom_point(aes(x = date_start, y = period, shape = religion)) +
  geom_text(data = timeline_of_jerusalem %>% filter(index == 1), aes(x = date_end, y = period, label = period_afr, hjust = -.1), size = 4) +
  scale_color_grey(start = 0.8, end = .2) +
  scale_x_continuous(breaks = seq(-2000, 2000, by = 500)) +
  ...
  labs(
    x = "Jaartal",
    y = "",
    caption = "Nota: Vertikale lyne dui begin en einde van die Kruistogte aan\nBron: Wikipedia: ‘Timeline of Jerusalem’"
  )
...
ggplot(data = world) +
  ...
  labs(x = "",
       y = "",
       caption = "Bron: Manning en Liu (2019)") +
  theme_void()
...
df_books_labels <- df_books_labels %>%
  arrange(desc(stat)) %>%
  mutate(country_afr = c(
    "Frankryk",
    "Groot-Brittanje",
    "Duitsland",
    "Italië",
...
df_books %>% 
  ...
  geom_line(alpha = .8) +
  geom_point() +
  geom_text_repel(data = df_books_labels, aes(period, stat, label = country_afr), hjust = -.1, size = 4.5) +
  scale_y_continuous(labels = scales::comma_format(big.mark = " ")) +
  theme(legend.position = "none") +
  # theme(axis.text.x = element_text(angle = 45)) +
  labs(y = "Vervaardiging van gedrukte boeke per halfeeu",
       x = "",
       caption = "Bron: Buringh en Van Zanden (2009)",
       lty = "")
...
# Kaapkolonie
# Yangzi
# Kaapkolonie (slegs setlaars)
# Europese leiers


df <- df %>%
  mutate(name_afr = case_when(
    name == "China" ~ "Missing",
    name == "China leader / Yangzi" ~ "Yangzi",
    name == "Europe leader" ~ "Europese leiers",
    name == "Cape Colony" ~ "Kaapkolonie",
    name == "Cape Colony (settlers only)" ~ "Kaapkolonie (slegs setlaars)"
  ))

jpeg(
    filename="graphs_afr/13.1.jpeg",
    width=8,
    height=6,
    units="in",
    res=1000)
...
df %>%
  filter(name != "China") %>%
  ggplot(aes(year, value, colour = name_afr, lty = name_afr, shape = name_afr)) +
  geom_point() +
  geom_line() +
  ...
  labs(
    x = NULL,
    y = "BBP per capita (1990 internasionale dollars)",
    colour = NULL,
    shape = NULL,
    lty = NULL,
    caption = "Bron: Fourie en Van Zanden (2013) & Broadberry, Guan en Li (2021)"
  )
...
df_gdp_england %>% 
  ...
  labs(#title = "GDP per capita in England",
       ...
       x = NULL,
       y = "BBP per capita in Engeland (2013-ponde)",
       caption = "Bron: Broadberry, Campbell, Overton en Van Leeuwen (2015) via die Bank van Engeland (2020)")
...
# Openbare en privaatskuld (nasionaal)
# Openbare skuld

abel_data <- abel_data %>% 
  mutate(series_afr = case_when(
    series == "Public and private debt (national)" ~ "Openbare en privaatskuld (nasionaal)",
    series == "Public debt" ~ "Openbare skuld"
  ))

jpeg(
    filename="graphs_afr/17.1.jpeg",
    width=8,
    height=6,
    units="in",
    res=1000)
...
abel_data %>%
  ggplot(aes(x, y, lty = series_afr)) +
  geom_line(size = 1.3) +
  scale_y_continuous(labels = scales::percent_format(scale = 1)) +
  labs(x = NULL,
       y = "Verhouding van skuld tot BBP",
       lty = "",
       caption = "Bron: Gwaindepi en Fourie (2020)") 
...
ggplot() +
  ...
  geom_smooth(data = height_mid, aes(x = year, y = height, fill = "Gemiddelde lengte van swart mans met 95% vertrouensinterval"), 
  ...
  labs(x = NULL,
       y = "Lengte (in cm)",
       colour = "Gemiddelde lengte van swart mans met 95% vertrouensinterval",
       caption = "Bron: Mpeta, Fourie en Inwood (2018)")
...
# Y-axis: BBP per capita (konstante 2010 US$)
# Verenigde State
# Voormalige USSR
# Bron: Maddison-projek databasis (2018)

jpeg(
    filename="graphs_afr/22.1.jpeg",
    width=8,
    height=6,
    units="in",
    res=1000)
...
df_mad_usa_fussr %>%
  ...
  geom_text(
    data = df_mad_usa_fussr %>%
      ...
      mutate(country_name = case_when(
        country_name == "United States" ~ "Verenigde State",
        TRUE ~ "Voormalige USSR"
      )),
    aes(year, stat, label = country_name, hjust = 1, vjust = -1.4),
    size = 5
  ) +
  scale_y_continuous(labels = scales::comma_format(big.mark = " ")) +
  ...
  expand_limits(x = 2015,
                y = 60000) +
  labs(
    y = "BBP per capita (konstante 2010-US$)",
    x = NULL,
    # title = "GDP per capita growth 1885-1995",
    caption = "Bron: Maddison-projek-databasis (2018)"
  ) +
  theme(legend.position = "none")
...
Y-axis: Lewensverwagting by geboorte
80 jaar
75 jaar
70 jaar
...
Romanië
Bulgarye
Nota: Die stippellyn toon die val van kommunisme aan
...
df_le_filter_labs <- df_le_filter_labs %>%
...
df_le_filter %>% 
  ...
  geom_text(data = df_le_filter_labs, aes(year, life_expectancy, label = entity_afr), hjust = .001, colour = "black") +
  geom_line(size = 1.4) +
  geom_vline(xintercept = 1990, lty = 2) +
  theme(legend.position = "none") +
  scale_color_grey() +
  scale_y_continuous(labels = scales::unit_format(suffix = " jaar", accuracy = 1)) +
  scale_x_continuous(limits = c(1960, 2034)) +
  labs(x = NULL,
       y = "Lewensverwagting by geboorte",
       caption = "Nota: Die stippellyn toon die val van kommunisme aan\nBron: Our World in Data")
...
a <- df_glf_a %>%
  ...
  scale_x_continuous(limits = c(1952, 1957)) +
  labs( # title = "Change in inventory of draft animals",
    x = NULL,
    y = "Inventaris van trekdiere"
  ) +
  theme(legend.position = "none")
...
b <- df_glf_b %>%
  ...
  labs( # title = "Change in cowhides sold",
    x = NULL,
    y = "Hoeveelheid beesvelle verkoop",
    caption = "Bron: Chen en Lan (2017)"
  ) +
  theme(legend.position = "none")

jpeg(
    filename="graphs_afr/23.1.jpeg",
    width=8,
    height=6,
    units="in",
    res=1000)
...
df_mad_comp %>%
  ...
  filter(!country_name %in% c("Albania"))
# 
# data.frame(
#   country_name = c(
#     "Bolivië",
#     "Peru", "Romanië",
#     "Sri Lanka", "Venezuela", "Indië", "Indonesië", "Brasilië",
#     "Meksiko", "Hongarye", "Portugal", "Griekeland",
#     "Finland", "Japan", "Italë", "Oostenryk (Austria)", "Swede",
#     "Spanje", "Suid-Afrika", "Kuba", "Uruguay",
#     "Frankryk", "Denemarke", "Kanada", "Nederland", "Noorweë",
#     "Switserland", "Duitsland (Germany)",
#     "Verenigde Koninkryk", "Verenigde State van Amerika", "Australië",
#     "België", "Nieu-Seeland"
#   )
# ) %>% as_tibble() %>% 
#   arrange(country_name)
...
afr_names <- readxl::read_excel("data/afrikaans_country_names_lookup.xlsx")

df_mad_comp <- df_mad_comp %>% 
  left_join(afr_names)

df_mad_comp <- df_mad_comp %>%
  mutate(continent_afr = case_when(
    continent == "Asia" ~ "Asië",
    continent == "Europe" ~ "Europa",
    continent == "Africa" ~ "Afrika",
    continent == "Americas" ~ "Amerikas",
    continent == "Oceania" ~ "Oseanië",
    TRUE ~ ""
  ))
...
df_mad_comp %>% 
  filter(indicator_code == "cgdppc",
         continent != "NA",
         country_code != "ARG") %>% 
  ggplot(aes(`1900`, `2000`, label = country_name_afr, shape = continent_afr), cex = 2.5) +
  geom_point(cex = 2.5) +
  geom_text_repel() +
  geom_label_repel(data = df_mad_comp %>% 
                     filter(indicator_code == "cgdppc",
                            continent != "NA",
                            country_code == "ARG"),
                   aes(`1900`, `2000`, label = "Argentinië", shape = continent_afr)) +
  # geom_abline() +
  scale_shape_manual(values = seq(15,20, by = 1)) +
  ...
  labs(x = "BBP per capita in 1900",
       y = "BBP per capita in 2000",
       shape = NULL,
       caption = "Bron: Maddison-projek databasis (2018)") +
  theme(legend.position = "bottom")
...
Freetown
Londen
Bron: Frankema en Van Waijenburg (2012)
...
Frankema <- Frankema %>%
  mutate(city_afr = case_when(
    city == "London" ~ "Londen",
    city == "Amsterdam" ~ "Amsterdam",
    city == "Accra" ~ "Accra",
    city == "Kyoto / Tokyo" ~ "Kioto/Tokio",
    city == "Freetown" ~ "Freetown",
    TRUE ~ "Beijing"
  ))

jpeg(
    filename="graphs_afr/25.1.jpeg",
    width=8,
    height=6,
    units="in",
    res=1000)
...
Frankema %>% 
  filter(x < 1960) %>% 
  ggplot(aes(x, y, colour = city_afr, shape = city_afr)) +
  geom_point() +
  ...
  labs(x = NULL,
       y = "Welvaartsratio",
       caption = "Bron: Frankema en Van Waijenburg (2012)")
...
s1 <- schooling_test_scores %>%
  ...
  geom_text_repel(data = schooling_test_scores %>% filter(country != "grey", panel == "a"), aes(x,y, label = country_afr)) +
...
s2 <- schooling_test_scores %>%
  ...
  geom_point() +
  geom_smooth(method ="lm", se = F, colour = "grey70") +
  geom_text_repel(data = schooling_test_scores %>% filter(country != "grey", panel == "b"), aes(x,y, label = country_afr)) +
  ...
  labs(y = "Groeikoers",
       x = "Toetsuitslae",
       caption = "Bron: Hanushek en Woessman (2016)")
...
world_trade_exports_constant_prices %>% 
  ...
  scale_y_continuous(labels = scales::comma_format(big.mark = " ")) + 
  scale_color_grey(end = .5) +
  labs(y = "Volume van uitvoere wêreldwyd (met 1913=100)", 
       x = NULL,
       # title = "GDP per capita growth 1885-1995",
       # subtitle = "Time series of value of world exports at constant prices, relative to 1913",
       caption = "Bron: Federico en Tena-Junguito (2016)",
       lty = "",
       colour = "") +
  theme(legend.position = "bottom")
...
df_transistors <- df_transistors %>% 
  mutate(designer = fct_lump(designer, 5, other_level = "Ander"))
...
df_transistors %>% 
  ...
  scale_y_log10(labels = scales::comma_format(big.mark = " ")) +
  labs(x = NULL,
       y = "Die getal transistors op geïntegreerde \nstroombaanskyfies (logaritmiese skaal)",
       shape = "Skyfie-ontwerper",
       colour = "Skyfie-ontwerper",
       caption = "Bron: Wikipedia: ‘Transistor Count’")
...
jpeg(
    filename="graphs_afr/32.1_v5.jpeg",
    width=8,
    height=6,
    units="in",
    res=1000)
...
any_bad_policy %>% 
  mutate(country = case_when(
    country == "World" ~ "Wêreld",
    country == "Latin America" ~ "Latyns-Amerika",
    TRUE ~ "Sub-Sahara Afrika"
  )) %>% 
  ...
  labs(#title = "Share of countries with any bad policy",
       lty = "Streek",
       alpha = "Streek",
       y= 'Persentasie van lande met "slegte beleid"',
       caption = "Bron: Weisbrot en Ray (2016)",
       x = NULL)
...
ggplot(data = df_sf) +
  ...
  labs(#title = "Share of deaths from HIV/AIDS - 2000",
       caption = "Bron: IMHE, Global Burden of Disease",
       x = NULL,
       y = NULL,
       fill = "Aandeel van sterftes van MIV/vigs (2000)")
...
georegions_2 <- georegions %>%
  mutate(AEZ_group = recode(AEZ_group,
                            Arid = "Woestyn",
                            Humid = "Tropiese reënwoud",
                            Subhumid = "Grasvlaktes",
                            Semiarid = "Grasvlaktes"))
...
ggplot() +
  ...
  geom_sf(data = georegions_simplified_3, aes(fill = "Gematigde streke"), alpha = .5, colour = NA) +
  geom_sf(data = highlands, aes(fill = "Hooglande"), alpha = .8, colour = NA) +
  # scale_fill_brewer(palette = "Greys") +
  scale_fill_manual(values = c("#F5F5F5", "#333333", "black", "#666666", "#CCCCCC")) +
  theme_map() +
  # theme(legend.position = "bottom") +
  labs(fill = "Ekologiese sone",
       caption = "Bron: Regional Centre for Mapping of Resources for Development (2018)") 
...
ggplot() +
  geom_sf(data = africa) +
  geom_sf(data = georegions_simplified, aes(fill = AEZ_group), alpha = .5, colour = NA) +
  geom_sf(data = highlands, aes(fill = "Hooglande"), alpha = .6, colour = NA) +
  # scale_fill_brewer(palette = "Greys") +
